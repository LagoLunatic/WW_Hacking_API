...
if not os.path.isdir("./asm_patches/patch_diffs"):
  os.mkdir("./asm_patches/patch_diffs")

custom_symbols = OrderedDict()
custom_symbols["sys/main.dol"] = OrderedDict()

with open("./asm_api/free_space_start_offsets.txt", "r") as f:
...
try:
  ...
  with open("./asm_api/asm_macros.asm") as f:
    asm_macros = f.read()
  ...
  if "main.asm" in all_asm_files:
    ...
    all_asm_files = ["main.asm"] + all_asm_files
  ...
  for patch_filename in all_asm_files:
    ...
    patch_path = os.path.join(".", "asm_patches", patch_filename)
  ...
  for patch_name, code_chunks_for_patch in code_chunks.items():
    ...
    diff_path = os.path.join(".", "asm_patches", "patch_diffs", patch_name + "_diff.txt")
